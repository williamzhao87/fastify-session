  ...
  return function saveSession (request, reply, payload, done) {
    ...
    options.store.set(session.sessionId, session, (err) => {
      ...
      reply.setCookie(
        ...
        session.cookie.options(isConnectionSecure(request, options.cookie.secureAutoHeaders))
      )
      done()
    })
  }
...
function ensureDefaults (options) {
  ...
  options.cookie.secureAutoHeaders = options.cookie.secureAutoHeaders || ''
  options.saveUninitialized = option(options, 'saveUninitialized', true)
  return options
}

function getRequestForwarded (request, secureAutoHeaders) {
  let protocol = ''

  secureAutoHeaders.split(',').map(header => {
    if (request.headers[header]) {
      protocol = request.headers[header]
    }
  })
  
  return protocol || request.headers['x-forwarded-proto'] || 'http'
}

function isConnectionSecure (request, secureAutoHeaders) {
  if (isConnectionEncrypted(request)) {
    return true
  }
  return getRequestForwarded(request, secureAutoHeaders) === 'https'
}
...
function shouldSaveSession (request, cookieOpts, saveUninitialized) {
  ...
  const forwarded = getRequestForwarded(request, cookieOpts.secureAutoHeaders)
