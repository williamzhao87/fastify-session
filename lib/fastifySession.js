...
function getRequestProto (request, secureAutoHeader) {
  return request.headers[secureAutoHeader || 'x-forwarded-proto'] || 'http'
}
...
function isConnectionSecure (request, secureAutoHeader) {
  ...
  return getRequestProto(request, secureAutoHeader) === 'https'
}
...
function shouldSaveSession (request, cookieOpts, saveUninitialized) {
  ...
  const forwardedProto = getRequestProto(request, cookieOpts.secureAutoHeader)
