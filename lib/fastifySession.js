  ...
  return function saveSession (request, reply, payload, done) {
    ...
    options.store.set(session.sessionId, session, (err) => {
      ...
      reply.setCookie(
        ...
        session.cookie.options(isConnectionSecure(request, options.cookie.secureAutoHeader))
      )
      done()
    })
  }
...
function ensureDefaults (options) {
  ...
  options.cookie.secureAutoHeader = options.cookie.secureAutoHeader || ''
  options.saveUninitialized = option(options, 'saveUninitialized', true)
  return options
}

function getRequestForwarded (request, secureAutoHeader) {
  return request.headers[secureAutoHeader] || request.headers['x-forwarded-proto'] || 'http'
}

function isConnectionSecure (request, secureAutoHeader) {
  if (isConnectionEncrypted(request)) {
    return true
  }
  return getRequestForwarded(request, secureAutoHeader) === 'https'
}
...
function shouldSaveSession (request, cookieOpts, saveUninitialized) {
  ...
  const forwarded = getRequestForwarded(request, cookieOpts.secureAutoHeader)
